...
public class Name extends Concept{
  ...
  public Set getNameInstances(){
    return Collections.EMPTY_SET;//class is abstract
  }
  public Concept getSingleNamedConcept(){
    Set nameInstances=getNameInstances();
    if(nameInstances.size()!=1)
      throw new RuntimeException("Name "+getName()+" has more than one meaning!");
    Iterator iterator=nameInstances.iterator();
    if(iterator.hasNext()){
      Concept concept=((NameInstance)iterator.next()).getConcept();
