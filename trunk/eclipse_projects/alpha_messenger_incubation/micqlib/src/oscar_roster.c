 ...
 *
 * $Id: oscar_roster.c,v 1.22 2005/05/17 21:02:56 kuhlmann Exp $
 */
...
JUMP_SNAC_F(SnacSrvReplyroster)
{
    ...
    for (i = k = l = 0; i < count; i++)
    {
        ...
        switch (re->type)
        {
            ...
            case ROSTER_TYPE_PRESENCE:
            case ROSTER_TYPE_NONCONT:
                /* TLV_UNKNIDLE   */
                /* TLV_PRIVACY    */
                /* TLV_VISIBILITY */
                /* TLV_ALLOWIDLE  */
    ...
    if (stmp)
    {
        if (!roster->import)
        {
            ...
            UDWORD da = now;
            SnacCliRosterentryadd (serv, "ImportTime", 0, 1, ROSTER_TYPE_IMPORTT, TLV_IMPORT, &da, 4);
        }
        if (!roster->ICQTIC)
            SnacCliRosterentryadd (serv, "ICQTIC", 0, 2, ROSTER_TYPE_ICQTIC, TLV_ICQTIC, "3608,0,0,0,60,null", 18);
